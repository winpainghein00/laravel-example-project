...
<body class="h-screen">
    <form action="{{ route('student.update',$student->id)}}" method="POST">
        ...
        <div class="max-w-[400px] mx-auto mt-6 flex flex-col gap-3 p-10 border border-black rounded-lg relative">
            ...
            <h1 class="font-bold text-lg font-serif text-center mb-6">Student Edit Form</h1>
            ...
            <div class="max-w-sm">
                ...
                <select name="course_id"
                    class="border py-3 px-3 pe-9 block w-full border-black rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600
                    @error('course_id')
                    border-red-500
                    @enderror
                    ">
                    ...
                    @foreach ($courses as $course)
                        <option value="{{ $course->id }}" @if ($course->id === $student->course_id) selected @endif>
                            {{ $course->name }}</option>
                    @endforeach
                </select>
                @error('course_id')
